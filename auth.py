...
async def create_payment_method(fullz: str, session: httpx.AsyncClient) -> str:
    try:
        ...
        data_login = {
            ...
            'log': 'jasonmurphy12099@gmail.com',   # Ganti akun login sesuai kamu
            'pwd': 'Jason1209',               # Ganti password sesuai kamu
            'wp-submit': 'Log In',
            'redirect_to': '',
        }
        ...
        data_stripe = {
            'type':'card',
            'billing_details[name]':'jason murphy',
            'billing_details[email]':'jasonmurphy12099@gmail.com',
            'card[number]': cc,
            'card[cvc]': cvv,
            'card[exp_month]': mes,
            'card[exp_year]': ano,
